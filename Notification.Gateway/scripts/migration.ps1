...
$startup = "..\src\NotificationGateway.Presentation"

$contexts = @(
    @{
        Name = "Notifications"
        Project = "..\src\NotificationGateway.Infrastructure"
...
# 2) Drop database once (using first context)
$first = $contexts[0]
RunStep ("Drop database (once) using context {0}" -f $first.Context) {
